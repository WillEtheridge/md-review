...
if [[ "$(go env GOVERSION)" != "go1.26.5" ]]; then
  echo "Milestone 2 requires Go 1.26.5" >&2
  exit 1
fi

if [[ "$(node --version)" != "v26.2.0" ]]; then
  echo "Milestone 2 requires Node.js 26.2.0" >&2
  exit 1
fi

if [[ "$(npm --version)" != "11.13.0" ]]; then
  echo "Milestone 2 requires npm 11.13.0" >&2
  exit 1
fi
...
if "${SCRIPT_DIRECTORY}/verify-release-assets.sh"; then
  echo "Milestone 2 expected release verification to remain blocked by deferred assets" >&2
  exit 1
fi

echo "Milestone 2 verification passed; deferred Agent Skill/release assets remain blocked."
