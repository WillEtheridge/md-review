...
function mappedBoundary(
  point: MappedDomPoint
): { sourceOffset: number | null; leafOffset: number } {
  return {
...
export function mapDomRange(
  ...
  const startPoint = nextMappedText(
  ...
  const endPoint = previousMappedText(
    range.endContainer,
    range.endOffset,
    root,
    model
