...
export function alignRenderedText(
  ...
  const boundaries: Array<number | null | undefined> = new Array(
    rendered.length + 1
  );
  ...
  for (const token of tokens) {
    if (!rendered.startsWith(token.value, renderedOffset)) {
      return {
        boundaries: new Array(rendered.length + 1).fill(null),
        error: `Rendered text diverged at UTF-16 offset ${renderedOffset}`
      };
    }
    ...
    setBoundary(
      boundaries,
      renderedOffset,
      absoluteSourceStart + token.rawStart
    );
    renderedOffset += token.value.length;
    setBoundary(
      boundaries,
      renderedOffset,
      absoluteSourceStart + token.rawEnd
    );
  ...
  if (renderedOffset !== rendered.length) {
    return {
      boundaries: new Array(rendered.length + 1).fill(null),
      error: `Rendered text has ${rendered.length - renderedOffset} unmatched code units`
    };
  }
...
export function fencedCodeContentSpan(
  ...
  const bodyStart =
    firstLineEnding + (raw.startsWith("\r\n", firstLineEnding) ? 2 : 1);
